...
function plumbing_parts_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = PLUMBING_PARTS_STORAGE['co_add_params']!=undefined ? PLUMBING_PARTS_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#plumbing_parts-customizer-styles-'+option).length > 0 ? jQuery('#plumbing_parts-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="plumbing_parts-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
