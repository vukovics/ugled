...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('plumbing_parts_sc_call_to_action_theme_setup')) {
	add_action( 'plumbing_parts_action_before_init_theme', 'plumbing_parts_sc_call_to_action_theme_setup' );
	function plumbing_parts_sc_call_to_action_theme_setup() {
		add_action('plumbing_parts_action_shortcodes_list', 		'plumbing_parts_sc_call_to_action_reg_shortcodes');
		if (function_exists('plumbing_parts_exists_visual_composer') && plumbing_parts_exists_visual_composer())
			add_action('plumbing_parts_action_shortcodes_list_vc','plumbing_parts_sc_call_to_action_reg_shortcodes_vc');
	}
}
...
if (!function_exists('plumbing_parts_sc_call_to_action')) {	
	function plumbing_parts_sc_call_to_action($atts, $content=null){	
		...
		$buttons = (!empty($link) || !empty($link2) 
						? '<div class="sc_call_to_action_buttons sc_item_buttons'.($need_columns && $style==2  ? ' column-2_'.esc_attr($need_columns) : '').'">'
							. (!empty($link) 
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_call_to_action'
					. (plumbing_parts_param_is_on($accent) ? ' sc_call_to_action_accented' : '')
...
//				. (plumbing_parts_param_is_on($accent) ? '<div class="content_wrap">' : '')
				...
				. '<div class="sc_call_to_action_info'.($need_columns ? ' column-'.esc_attr($need_columns-2).'_'.esc_attr($need_columns) : '').'">'
					. (!empty($subtitle) ? '<h6 class="sc_call_to_action_subtitle sc_item_subtitle">' . trim(plumbing_parts_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_call_to_action_title sc_item_title">' . trim(plumbing_parts_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_call_to_action_descr sc_item_descr">' . trim(plumbing_parts_strmacros($description)) . '</div>' : '')
					. ($style==1 ? $buttons : '')
					. ($style==3 ? $buttons : '')
				. '</div>'
				. ($style==2 && $align!='right' ? $buttons : '')
...
//				. (plumbing_parts_param_is_on($accent) ? '</div>' : '')
		...
		return apply_filters('plumbing_parts_shortcode_output', $output, 'trx_call_to_action', $atts, $content);
	}
	plumbing_parts_require_shortcode('trx_call_to_action', 'plumbing_parts_sc_call_to_action');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'plumbing_parts_sc_call_to_action_reg_shortcodes' ) ) {
	//add_action('plumbing_parts_action_shortcodes_list', 'plumbing_parts_sc_call_to_action_reg_shortcodes');
	function plumbing_parts_sc_call_to_action_reg_shortcodes() {
	
		plumbing_parts_sc_map("trx_call_to_action", array(
			"title" => esc_html__("Call to action", 'plumbing-parts'),
			"desc" => wp_kses_data( __("Insert call to action block in your page (post)", 'plumbing-parts') ),
			"decorate" => true,
			"container" => true,
			"params" => array(
				"title" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'plumbing_parts_sc_call_to_action_reg_shortcodes_vc' ) ) {
	//add_action('plumbing_parts_action_shortcodes_list_vc', 'plumbing_parts_sc_call_to_action_reg_shortcodes_vc');
	function plumbing_parts_sc_call_to_action_reg_shortcodes_vc() {
	
		vc_map( array(
			"base" => "trx_call_to_action",
			"name" => esc_html__("Call to Action", 'plumbing-parts'),
			"description" => wp_kses_data( __("Insert call to action block in your page (post)", 'plumbing-parts') ),
		...
		class WPBakeryShortCode_Trx_Call_To_Action extends PLUMBING_PARTS_VC_ShortCodeCollection {}
	}
}
?>