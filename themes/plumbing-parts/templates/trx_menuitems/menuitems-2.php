...
if ( !function_exists( 'plumbing_parts_template_menuitems_2_theme_setup' ) ) {
	add_action( 'plumbing_parts_action_before_init_theme', 'plumbing_parts_template_menuitems_2_theme_setup', 1 );
	function plumbing_parts_template_menuitems_2_theme_setup() {
		plumbing_parts_add_template(array(
			'layout' => 'menuitems-2',
			'template' => 'menuitems-2',
			'mode'   => 'menuitems',
			'title'  => esc_html__('MenuItems /Style 2/', 'plumbing-parts'),
			'thumb_title'  => esc_html__('Fullwidth image (crop)', 'plumbing-parts'),
			'w'		 => 1170,	//1920,
			'h'		 => 659		//698
		));
	}
...
// Template output
if ( !function_exists( 'plumbing_parts_template_menuitems_2_output' ) ) {
	function plumbing_parts_template_menuitems_2_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if ($post_options['menuitem_image']) {
					...
					plumbing_parts_show_layout($post_options['menuitem_image']);
					if ( $post_options['menuitem_spicylevel'] != 'inherit' && $post_options['menuitem_spicylevel'] != '0' && $post_options['menuitem_spicylevel'] != 'NaN' ) {
						?>
						<div class="sc_menuitem_spicy menuitem_spicylevel_<?php plumbing_parts_show_layout($post_options['menuitem_spicylevel']); ?> ">
							<span class="icon-fire"></span> Spicy Level <?php plumbing_parts_show_layout($post_options['menuitem_spicylevel']); ?>/5
						</div>
						<?php
					}
					?></div><?php
				...
				if ( $post_data['post_title'] || ( $post_options['menuitem_price'] != 'inherit' && strlen($post_options['menuitem_price']) != 0 ) ) {
					...
					<div class="sc_menuitem_box_title">
						...
						if ($post_data['post_title']) {
							?><div class="sc_menuitem_title"><strong><?php plumbing_parts_show_layout($post_data['post_title']);?></strong></div><?php
						}
						if ( $post_options['menuitem_price'] != 'inherit' && strlen($post_options['menuitem_price']) != 0 ) {
							?><div class="sc_menuitem_price"><?php plumbing_parts_show_layout($post_options['menuitem_price']); ?></div><?php
						}
						?>
						<div class="cL"></div>
					</div>
				...
				if ($post_data['post_content']) {
					...
					<div class="sc_menuitem_content">
						...
						<?php plumbing_parts_show_layout($post_data['post_content']); ?>
					</div>
					<?php
				}
				...
				if (   ( $post_options['menuitem_calories'] != 'inherit'		&& strlen($post_options['menuitem_calories']) != 0 )
					...
					<div class="sc_menuitem_nutritions">
						...
						<ul class="sc_menuitem_nutritions_list">
							<?php
							if ( $post_options['menuitem_calories'] != 'inherit' && strlen($post_options['menuitem_calories']) != 0 )
								echo '<li>' . esc_html__('Calories: ', 'plumbing-parts') . '<span>' . trim($post_options['menuitem_calories']) . esc_html__('Kcal', 'plumbing-parts') . '</span></li>';
							if ( $post_options['menuitem_cholesterol'] != 'inherit' && strlen($post_options['menuitem_cholesterol']) != 0 ) {
								echo '<li>' . esc_html__('Cholesterol: ', 'plumbing-parts') . '<span>' . trim($post_options['menuitem_cholesterol']) . esc_html__('mg', 'plumbing-parts') . '</span></li>';
							}
							if ( $post_options['menuitem_fiber'] != 'inherit' && strlen($post_options['menuitem_fiber']) != 0 ) {
								echo '<li>' . esc_html__('Fiber: ', 'plumbing-parts') . '<span>' . trim($post_options['menuitem_fiber']) . esc_html__('g', 'plumbing-parts') . '</span></li>';
							}
							if ( $post_options['menuitem_sodium'] != 'inherit' && strlen($post_options['menuitem_sodium']) != 0 ) {
								echo '<li>' . esc_html__('Sodium: ', 'plumbing-parts') . '<span>' . trim($post_options['menuitem_sodium']) . esc_html__('mg', 'plumbing-parts') . '</span></li>';
							}
							if ( $post_options['menuitem_carbohydrates'] != 'inherit' && strlen($post_options['menuitem_carbohydrates']) != 0 ) {
								echo '<li>' . esc_html__('Carbohydrates: ', 'plumbing-parts') . '<span>' . trim($post_options['menuitem_carbohydrates']) . esc_html__('g', 'plumbing-parts') . '</span></li>';
							} 
							if ( $post_options['menuitem_fat'] != 'inherit' && strlen($post_options['menuitem_fat']) != 0 ) { 
								echo '<li>' . esc_html__('Fat: ', 'plumbing-parts') . '<span>' . trim($post_options['menuitem_fat']) . esc_html__('g', 'plumbing-parts') . '</span></li>';
							}
							if ( $post_options['menuitem_protein'] != 'inherit' && strlen($post_options['menuitem_protein']) != 0 ) {
								echo '<li>' . esc_html__('Protein: ', 'plumbing-parts') . '<span>' . trim($post_options['menuitem_protein']) . esc_html__('g', 'plumbing-parts') . '</span></li>';
							}
							?>
						</ul>
						<div class="cL"></div>
				...
				<div class="sc_menuitem_more">
					<?php if ( $post_options['menuitem_product'] != 'inherit' ) { ?>
						<a class="sc_button sc_button_square sc_button_style_filled sc_button_size_small margin_right_tiny" href="<?php echo get_permalink($post_options['menuitem_product']); ?>"><?php esc_html_e('ORDER', 'plumbing-parts'); ?></a>
					<?php } ?>
					<a class="sc_button sc_button_square sc_button_style_filled sc_button_size_small" href="<?php echo esc_url($post_data['post_comments_link']); ?>"><?php esc_html_e('POST COMMENT', 'plumbing-parts'); ?></a>
					<div class="cL"></div>
				</div>
