...
if ( !function_exists( 'plumbing_parts_template_single_matches_theme_setup' ) ) {
	add_action( 'plumbing_parts_action_before_init_theme', 'plumbing_parts_template_single_matches_theme_setup', 1 );
	function plumbing_parts_template_single_matches_theme_setup() {
		plumbing_parts_add_template(array(
			'layout' => 'single-matches',
			'mode'   => 'matches',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single matches', 'plumbing-parts'),
...
// Template output
if ( !function_exists( 'plumbing_parts_template_single_matches_output' ) ) {
	function plumbing_parts_template_single_matches_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && plumbing_parts_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_html($title_tag); ?> itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php plumbing_parts_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
		<?php 
		}
		...
		if (!$post_data['post_protected']) {
			...
			get_template_part(plumbing_parts_get_file_slug('templates/trx_matches/_parts/match-info.php'));
		}
		...
		if ($post_data['post_protected']) { 
			...
			if ( plumbing_parts_get_custom_option('show_post_tags') == 'yes' && !empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) {
				...
				<div class="post_info post_info_bottom">
					<span class="post_info_item post_info_tags"><?php esc_html_e('Tags:', 'plumbing-parts'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
				</div>
				<?php 
			}
		} 
		...
		?>
			<div class="match_preview"><a href="<?php echo esc_url($match_preview); ?>"><?php echo esc_html__("Go to the announcement &#8594;", 'plumbing-parts'); ?></a></div>
		<?php
		} 
		...
		$sidebar_present = !plumbing_parts_param_is_off(plumbing_parts_get_custom_option('show_sidebar_main'));
		if (!$sidebar_present) plumbing_parts_close_wrapper();	// .post_item
