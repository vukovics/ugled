...
if ( !function_exists( 'plumbing_parts_template_team_3_theme_setup' ) ) {
	add_action( 'plumbing_parts_action_before_init_theme', 'plumbing_parts_template_team_3_theme_setup', 1 );
	function plumbing_parts_template_team_3_theme_setup() {
		plumbing_parts_add_template(array(
			'layout' => 'team-3',
			'template' => 'team-3',
			'mode'   => 'team',
			/*'container_classes' => 'sc_slider_nopagination sc_slider_controls sc_slider_controls_side',*/
			'title'  => esc_html__('Team /Style 3/', 'plumbing-parts'),
...
// Template output
if ( !function_exists( 'plumbing_parts_template_team_3_output' ) ) {
	function plumbing_parts_template_team_3_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_team_item_avatar"><?php plumbing_parts_show_layout($post_options['photo']); ?>
					<div class="sc_team_item_hover">
						<div class="sc_team_item_socials"><?php plumbing_parts_show_layout($post_options['socials']); ?></div>
					</div>
				</div>
