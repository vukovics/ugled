...
if ( !function_exists( 'plumbing_parts_template_single_standard_theme_setup' ) ) {
	add_action( 'plumbing_parts_action_before_init_theme', 'plumbing_parts_template_single_standard_theme_setup', 1 );
	function plumbing_parts_template_single_standard_theme_setup() {
		plumbing_parts_add_template(array(
			'layout' => 'single-standard',
			'mode'   => 'single',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single standard', 'plumbing-parts'),
...
// Template output
if ( !function_exists( 'plumbing_parts_template_single_standard_output' ) ) {
	function plumbing_parts_template_single_standard_output($post_options, $post_data) {
        ...
        if ($show_title && $post_options['location'] != 'center' && !empty($post_data['post_title'])) {
            ?><h3 class="post_title_single"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php plumbing_parts_show_layout($post_data['post_title']); ?></a></h3><?php
        }
