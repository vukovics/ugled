...
if ( !function_exists( 'plumbing_parts_template_services_1_theme_setup' ) ) {
	add_action( 'plumbing_parts_action_before_init_theme', 'plumbing_parts_template_services_1_theme_setup', 1 );
	function plumbing_parts_template_services_1_theme_setup() {
		plumbing_parts_add_template(array(
			'layout' => 'services-1',
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => esc_html__('Services /Style 1/', 'plumbing-parts'),
...
// Template output
if ( !function_exists( 'plumbing_parts_template_services_1_output' ) ) {
	function plumbing_parts_template_services_1_output($post_options, $post_data) {
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !plumbing_parts_param_is_off($post_options['readmore'])) {
								?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_services_item_readmore sc_button"><?php plumbing_parts_show_layout($post_options['readmore']); ?></a><?php
							}
						}
						?>
					</div>
