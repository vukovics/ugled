...
// Theme init
if (!function_exists('plumbing_parts_widget_recent_posts_theme_setup')) {
	add_action( 'plumbing_parts_action_before_init_theme', 'plumbing_parts_widget_recent_posts_theme_setup', 1 );
	function plumbing_parts_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('plumbing_parts_action_shortcodes_list',	'plumbing_parts_widget_recent_posts_reg_shortcodes');
		if (function_exists('plumbing_parts_exists_visual_composer') && plumbing_parts_exists_visual_composer())
			add_action('plumbing_parts_action_shortcodes_list_vc','plumbing_parts_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('plumbing_parts_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'plumbing_parts_widget_recent_posts_load' );
	function plumbing_parts_widget_recent_posts_load() {
		register_widget('plumbing_parts_widget_recent_posts');
	}
}

// Widget Class
class plumbing_parts_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'plumbing-parts'));
		parent::__construct( 'plumbing_parts_widget_recent_posts', esc_html__('Plumbing Parts - Recent Posts', 'plumbing-parts'), $widget_ops );
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'plumbing-parts')), plumbing_parts_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'plumbing_parts_sc_widget_recent_posts' ) ) {
	function plumbing_parts_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'plumbing_parts_widget_recent_posts';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, plumbing_parts_prepare_widgets_args(plumbing_parts_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('plumbing_parts_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	plumbing_parts_require_shortcode("trx_widget_recent_posts", "plumbing_parts_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('plumbing_parts_widget_recent_posts_reg_shortcodes_vc')) {
	function plumbing_parts_widget_recent_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Posts", 'plumbing-parts'),
				"description" => wp_kses_data( __("Insert recent posts list with thumbs and post's meta", 'plumbing-parts') ),
