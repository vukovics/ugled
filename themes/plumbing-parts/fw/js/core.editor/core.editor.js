jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(PLUMBING_PARTS_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == PLUMBING_PARTS_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(PLUMBING_PARTS_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				plumbing_parts_message_success('', PLUMBING_PARTS_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(PLUMBING_PARTS_STORAGE['strings']['editor_caption_close']);
			} else {
				plumbing_parts_message_warning(rez.error, PLUMBING_PARTS_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		plumbing_parts_message_confirm(PLUMBING_PARTS_STORAGE['strings']['editor_delete_post'], PLUMBING_PARTS_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(PLUMBING_PARTS_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					plumbing_parts_message_success('', PLUMBING_PARTS_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = PLUMBING_PARTS_STORAGE['site_url'];
						}, 1000);
				} else {
					plumbing_parts_message_warning(rez.error, PLUMBING_PARTS_STORAGE['strings']['editor_delete_error']);
				}
			});
