// Plumbing Parts Customizer script
...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.plumbing_parts_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.plumbing_parts_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && plumbing_parts_get_cookie('to_demo_fixed')==1) plumbing_parts_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.plumbing_parts_options_field_info .to_demo_pin', function(e) {
		plumbing_parts_options_fix_demo(jQuery(this).parents('.plumbing_parts_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	plumbing_parts_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.plumbing_parts_options_input,.plumbing_parts_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.plumbing_parts_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		plumbing_parts_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.plumbing_parts_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.plumbing_parts_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .plumbing_parts_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.plumbing_parts_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? PLUMBING_PARTS_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.plumbing_parts_options_tab').find('> .plumbing_parts_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="plumbing_parts_options_schemes_order" class="plumbing_parts_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function plumbing_parts_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.plumbing_parts_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.plumbing_parts_options').find('>.plumbing_parts_options_field_info');
...
function plumbing_parts_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.plumbing_parts_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	plumbing_parts_message_confirm(PLUMBING_PARTS_STORAGE['to_strings']['scheme_delete_confirm'], PLUMBING_PARTS_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'plumbing_parts_options_scheme_delete',
			nonce: PLUMBING_PARTS_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.plumbing_parts_options_tab').find('> .plumbing_parts_options_schemes_order').val()
		};
		setTimeout(function(){
			plumbing_parts_message_info(PLUMBING_PARTS_STORAGE['to_strings']['recompile_styles'], PLUMBING_PARTS_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(PLUMBING_PARTS_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				plumbing_parts_message_success(PLUMBING_PARTS_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+PLUMBING_PARTS_STORAGE['to_strings']['reload_page'], PLUMBING_PARTS_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				plumbing_parts_message_warning(PLUMBING_PARTS_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, PLUMBING_PARTS_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function plumbing_parts_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.plumbing_parts_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	plumbing_parts_message_confirm(PLUMBING_PARTS_STORAGE['to_strings']['scheme_copy_confirm'], PLUMBING_PARTS_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'plumbing_parts_options_scheme_copy',
			nonce: PLUMBING_PARTS_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.plumbing_parts_options_tab').find('> .plumbing_parts_options_schemes_order').val()
		};
		...
		jQuery.post(PLUMBING_PARTS_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				plumbing_parts_message_success(PLUMBING_PARTS_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+PLUMBING_PARTS_STORAGE['to_strings']['reload_page'], PLUMBING_PARTS_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				plumbing_parts_message_warning(PLUMBING_PARTS_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, PLUMBING_PARTS_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function plumbing_parts_options_fix_demo(info) {
	var info2 = info.parents('.plumbing_parts_options').toggleClass('to_demo_fixed').find('>.plumbing_parts_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.plumbing_parts_options').append(clonedObj);
		info2 = info.parents('.plumbing_parts_options').find('>.plumbing_parts_options_field_info');
	}
	plumbing_parts_set_cookie('to_demo_fixed', jQuery('.plumbing_parts_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function plumbing_parts_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.plumbing_parts_options_field_info').data('scheme');
