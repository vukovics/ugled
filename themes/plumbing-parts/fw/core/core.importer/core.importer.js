// Plumbing Parts Importer script
...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_demo_type input[type="radio"]').on('change', function() {
		...
		var data = {
			ajax_nonce: PLUMBING_PARTS_STORAGE['ajax_nonce'],
			action: 'plumbing_parts_importer_get_list_pages',
			demo_type: type
		};
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		PLUMBING_PARTS_STORAGE['importer_error_messages'] = '';
		PLUMBING_PARTS_STORAGE['importer_ignore_errors'] = true;
		plumbing_parts_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function plumbing_parts_importer_do_action(steps, idx) {
	"use strict";
	...
	var data = {
		ajax_nonce: PLUMBING_PARTS_STORAGE['ajax_nonce'],
		action: 'plumbing_parts_importer_start_import',
	...
	jQuery.post(PLUMBING_PARTS_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '' || PLUMBING_PARTS_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '') 
				PLUMBING_PARTS_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				plumbing_parts_importer_do_action(steps, idx);
			} else {
				if (PLUMBING_PARTS_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + PLUMBING_PARTS_STORAGE['importer_error_msg'] + '</h4>' + PLUMBING_PARTS_STORAGE['importer_error_messages']);
				} else {
